...
bool LLParser::ValidateEndOfModule() {
  ...
  for (const auto &RAG : ForwardRefAttrGroups) {
    ...
    if (Function *Fn = dyn_cast<Function>(V)) {
      ...
      AS = AS.addAttributes(
          Context, AttributeSet::FunctionIndex,
          AttributeSet::get(Context, AttributeSet::FunctionIndex, FnAttrs));
      Fn->setAttributes(AS);
...
bool LLParser::ParseParameterList(SmallVectorImpl<ParamInfo> &ArgList,
  ...
  while (Lex.getKind() != lltok::rparen) {
    ...
    ArgList.push_back(ParamInfo(
        ArgLoc, V, AttributeSet::get(V->getContext(), AttrIndex++, ArgAttrs)));
  }
...
#undef VISIT_MD_FIELDS
  ...
  Result = GET_OR_DISTINCT(DINamespace, (Context, scope.Val, file.Val, name.Val,
                                         line.Val, exportSymbols.Val));
  return false;
}
...
bool LLParser::ParseFunctionHeader(Function *&Fn, bool isDefine) {
  ...
  if (RetAttrs.hasAttributes())
    Attrs.push_back(AttributeSet::get(RetType->getContext(),
                                      AttributeSet::ReturnIndex, RetAttrs));
  ...
  if (FuncAttrs.hasAttributes())
    Attrs.push_back(AttributeSet::get(RetType->getContext(),
                                      AttributeSet::FunctionIndex, FuncAttrs));
...
bool LLParser::ParseBasicBlock(PerFunctionState &PFS) {
  ...
  if (!BB)
    return Error(NameLoc, "unable to create block named '" + Name + "'");
...
int LLParser::ParseInstruction(Instruction *&Inst, BasicBlock *BB,
  ...
  case lltok::kw_shl: {
    ...
    if (ParseArithmetic(Inst, PFS, KeywordVal, 1))
      return true;
...
int LLParser::ParseAtomicRMW(Instruction *&Inst, PerFunctionState &PFS) {
  ...
  AtomicRMWInst *RMWI = new AtomicRMWInst(Operation, Ptr, Val, Ordering, Scope);
